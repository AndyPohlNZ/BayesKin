...
WORKING_DIR = "/media/data/ActiveProjects/P01_BayesianKinematics/PublishedCode/BayesKin/src" # Replace with appropiate location of working directory on local PC
...
# midpoint of range explored by Pataky et al. 2019
...
nits = 1000 # number of iterations to perform

seeds = 1:nits
i = 1
while (length(list.files('./SingleLink_Mdl/Bayes_p3/')) < nits){
    files = list.files(path = paste0("./SingleLink_Mdl/Bayes_p3/"))
    ...
    if(!(seed %in% processed_seeds)){
        set.seed(seed)
        print(sprintf("Iteration %.0f of %.0f  -  seed = %.0f",length(list.files('./SingleLink_Mdl/Bayes_p3/')), nits, seed))
        # gen obs
        ...
        r_mdl_file_name = paste0('./SingleLink_Mdl/LS/', as.character(as.numeric(seed)))
        ...
        r_mdl_file_name = paste0('./SingleLink_Mdl/Bayes_p1/', as.character(as.numeric(seed)))
        r_mdl_file_name = paste0(r_mdl_file_name, '_')
        ...
        r_mdl_file_name = paste0('./SingleLink_Mdl/Bayes_p2/', as.character(as.numeric(seed)))
        r_mdl_file_name = paste0(r_mdl_file_name, '_')
        r_mdl_file_name = paste0(r_mdl_file_name, mdl_type)
        r_mdl_file_name = paste0(r_mdl_file_name, '.rda')
        print(sprintf("Saving file %s", r_mdl_file_name))
        ...
        r_mdl_file_name = paste0('./SingleLink_Mdl/Bayes_p3/', as.character(as.numeric(seed)))
        r_mdl_file_name = paste0(r_mdl_file_name, '_')
        r_mdl_file_name = paste0(r_mdl_file_name, mdl_type)
        r_mdl_file_name = paste0(r_mdl_file_name, '.rda')
        print(sprintf("Saving file %s", r_mdl_file_name))
