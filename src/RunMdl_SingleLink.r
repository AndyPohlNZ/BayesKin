...
seeds = 1:nits
i = 1
while (length(list.files('./SingleLink_Mdl/Bayes_p5/')) < nits){
    files = list.files(path = paste0("./SingleLink_Mdl/Bayes_p5/"))
    ...
    if(!(seed %in% processed_seeds)){
        set.seed(seed)
        print(sprintf("Iteration %.0f of %.0f  -  seed = %.0f",length(list.files('./SingleLink_Mdl/Bayes_p5/')), nits, seed))
        # gen obs
        ...
        r_mdl_file_name = paste0('./SingleLink_Mdl/Bayes_p4/', as.character(as.numeric(seed)))
        r_mdl_file_name = paste0(r_mdl_file_name, '_')
        r_mdl_file_name = paste0(r_mdl_file_name, mdl_type)
        r_mdl_file_name = paste0(r_mdl_file_name, '.rda')
        print(sprintf("Saving file %s", r_mdl_file_name))
        ...
        r_mdl_file_name = paste0('./SingleLink_Mdl/Bayes_p5/', as.character(as.numeric(seed)))
        r_mdl_file_name = paste0(r_mdl_file_name, '_')
        r_mdl_file_name = paste0(r_mdl_file_name, mdl_type)
        r_mdl_file_name = paste0(r_mdl_file_name, '.rda')
        print(sprintf("Saving file %s", r_mdl_file_name))
