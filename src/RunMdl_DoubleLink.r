...
INIT_TYPE = 'random' # specify init type
...
while (length(list.files('./DoubleLink_Mdl/Bayes_p3/')) < nits){
    files = list.files(path = paste0("./DoubleLink_Mdl/Bayes_p3/"))
    processed_seeds = as.numeric(gsub("(.+?)(\\_.*)", "\\1", files))
    #set seed
    seed = seeds[i]
    if(!(seed %in% processed_seeds)){
        set.seed(seed)
        print(sprintf("Iteration %.0f of %.0f  -  seed = %.0f",length(list.files('./DoubleLink_Mdl/Bayes_p3/')), nits, seed))
        # gen obs
